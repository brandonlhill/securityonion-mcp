...
def get_mappings(index: str) -> dict:
    """Retrieve field mappings for a specific Elasticsearch index."""
...
@so_elasticsearch_mcp.tool()
def semantic_search(index: str, field: str, text: str, size: int = 10) -> dict:
    """Perform a semantic search on a specified field using the provided text."""
    query = {
        "match": {
            field: {
                "query": text
